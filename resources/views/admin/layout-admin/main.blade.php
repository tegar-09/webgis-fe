<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>@yield('title')</title>

    <link rel="shortcut icon" href="{{ asset('assets-admin/compiled/svg/favicon.svg') }}" type="image/x-icon">
    <link rel="shortcut icon"
        href="data:image/png;base64,"
        type="image/png">
    <link rel="stylesheet" href="{{ asset('assets-admin/compiled/css/app.css') }}">
    <link rel="stylesheet" href="{{ asset('assets-admin/compiled/css/style.css') }}"> {{-- css buat sendiri --}}
    <link rel="stylesheet" href="{{ asset('assets-admin/compiled/css/app-dark.css') }}">
    <link rel="stylesheet" href="{{ asset('assets-admin/compiled/css/iconly.css') }}">

    <!-- Font Awesome menampilkan icon -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
    <!-- Bootstrap -->
    {{-- <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css"> --}}

    {{-- library leaflet.js --}}
    <link rel="stylesheet" href="https://unpkg.com/leaflet/dist/leaflet.css" />
    {{-- <link rel="stylesheet" href="{{ asset('assets-admin/compiled/css/marker-cluster/MarkerCluster.css') }}">
    <link rel="stylesheet" href="{{ asset('assets-admin/compiled/css/marker-cluster/MarkerCluster.Default.css') }}"> --}}
    <link rel="stylesheet" href="https://unpkg.com/leaflet.markercluster@1.5.1/dist/MarkerCluster.css" />
    <link rel="stylesheet" href="https://unpkg.com/leaflet.markercluster@1.5.1/dist/MarkerCluster.Default.css" />

    {{-- js leaflet --}}
    <script src="https://unpkg.com/leaflet/dist/leaflet.js"></script>
    <script src="https://unpkg.com/leaflet.markercluster@1.5.1/dist/leaflet.markercluster.js"></script>
    <script src="https://unpkg.com/leaflet.featuregroup.subgroup@1.0.2/dist/leaflet.featuregroup.subgroup.js"></script>

    {{-- <script src="{{ asset('assets-admin/compiled/geojson/batas_desa.geojson') }}"></script> --}}
    {{-- <script src="{{ asset('assets-admin/compiled/geojson/batas_kecamatan.geojson') }}"></script> --}}

</head>

<body>
    <script src="{{ asset('assets-admin/static/js/initTheme.js') }}"></script>

    <div id="app">
        {{-- menu sidebar --}}
        <aside>
            @include('admin.layout-admin.sidebar')
        </aside>

        {{-- isi konten --}}
        <div id="main">
            <header class="mb-3">
                <a href="#" class="burger-btn d-block d-xl-none">
                    <i class="bi bi-justify fs-3"></i>
                </a>
            </header>

            <main>
                @yield('content')
            </main>

            {{-- footer --}}
            <footer>
                @include('admin.layout-admin.footer')
            </footer>
        </div>
    </div>

    <script src="{{ asset('assets-admin/static/js/components/dark.js') }}"></script>
    <script src="{{ asset('assets-admin/extensions/perfect-scrollbar/perfect-scrollbar.min.js') }}"></script>
    <script src="{{ asset('assets-admin/compiled/js/app.js') }}"></script>
    {{-- <script src="{{ asset('assets-admin/compiled/js/marker-cluster/leaflet.markercluster.js') }}"></script>
    <script src="{{ asset('assets-admin/compiled/js/feature-group/leaflet.featuregroup.subgroup.js') }}"></script> --}}

    <!-- Need: Apexcharts -->
    <script src="https://cdn.jsdelivr.net/npm/apexcharts"></script>
    <!-- Swets Alert -->
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>

    <!-- leaflet.js -->

    {{-- <script src="{{ asset('assets-admin/extensions/apexcharts/apexcharts.min.js') }}"></script>
    <script src="{{ asset('assets-admin/static/js/pages/dashboard.js') }}"></script> --}}

</body>

</html>
